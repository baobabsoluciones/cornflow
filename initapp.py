...
CORNFLOW_DB_CONN = os.getenv("CORNFLOW_DB_CONN","postgres://"+CORNFLOW_DB_USER+":"+CORNFLOW_DB_PASSWORD+"@"+CORNFLOW_DB_HOST+":"+CORNFLOW_DB_PORT+"/"+CORNFLOW_DB)
...
CORNFLOW_SERVICE_EMAIL = os.getenv("CORNFLOW_SERVICE_EMAIL","service_user@cornflow.com")
...
with app.app_context():
  ...
  if AUTH == 1 :
    ...
    create_user_with_role(CORNFLOW_ADMIN_USER,CORNFLOW_ADMIN_EMAIL,CORNFLOW_ADMIN_PWD,"admin",ADMIN_ROLE,verbose=1)
    # create cornflow service user
    create_user_with_role(CORNFLOW_SERVICE_USER,CORNFLOW_SERVICE_EMAIL,CORNFLOW_SERVICE_PWD,"serviceuser",SERVICE_ROLE,verbose=1)

# execute gunicorn application from gunicorn pkg....working on it
WSGIApplication.get_config_from_filename(self=WSGIApplication,filename="cornflow/gunicorn.py")
WSGIApplication.run()