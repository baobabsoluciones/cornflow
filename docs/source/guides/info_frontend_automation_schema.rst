...
* Nunca se asocian a ambos (section y group) a la vez, ya que si pertenecen a un grupo, ya están dentro de la sección a la que ese grupo pertenece (si es que el grupo pertenece a una sección)
...
* Recibe lista de objetos (sin IDs)
* Si existe un registro con los mismos campos únicos, se actualiza. Si no existe, se crea.
* Operación atómica:
...
.. list-table::
    ...
    * - Bloque
      - Responsabilidad
    * - ``available_automations``
      - Organización visual y funcional en frontend
    * - ``definitions``
      - Modelos de datos (OpenAPI / JSON Schema)
    * - ``paths``
      - Endpoints REST y operaciones disponibles
