...
if extension=='.tsp':
  ...
  my_instance = TspApp.instance.from_json(file_name)
else:
...
# TODO: Ideally, we should not solve the problem inside the report, we should already have an example of problem solved.
...
The distance distribution is shown in the histogram of @fig-distance-dist.
...
See @fig-network for a representation of the network distances.
...
See @fig-tour for a representation of the solution tour. Number in links show the actual distance. Colors are kept from the network representation of the instance.
