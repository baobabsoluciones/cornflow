...
class APIGenerator:
    def __init__(self, schema_path, app_name, output_path=None, options=None, one_table=False, name_table=None):
    ...
    def new_model(self, table_name):
        ...
        mg = ModelGenerator(class_name, self.schema, parents_class, table_name, self.name)
    ...
    def new_endpoint(self, table_name, model_name, schemas_names):
        filename = os.path.join(self.endpoint_path, self.name + '_' + table_name + '.py')
        class_name_all = self.snake_to_camel(self.name + '_' + table_name + '_endpoint')
        class_name_details = self.snake_to_camel(self.name + '_' + table_name + '_details_endpoint')
    ...
    def new_schemas(self, table_name):
        ...
        class_name_edit = self.snake_to_camel(self.name + '_' + table_name + '_edit_request')
        class_name_post = self.snake_to_camel(self.name + '_' + table_name + '_post_request')
        ...
        return {'one': class_name_one, 'editRequest': class_name_edit, 'postRequest': class_name_post}
