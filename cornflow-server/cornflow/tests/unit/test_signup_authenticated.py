"""
Unit test for the sign-up endpoints (public and authenticated)
"""
...
class TestSignUpPublic(TestCase):
    ...
    def test_public_signup_no_auth_header_required(self):
        """Test that no authorization header is required for public signup"""
        payload = self.data
...
class TestSignUpAuthenticated(TestCase):
    ...
    def create_app(self):
        with patch("cornflow.config.DefaultConfig.SIGNUP_ACTIVATED", 2):
            app = create_app("testing")
        return app
